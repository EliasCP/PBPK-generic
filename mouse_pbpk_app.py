...
def compute_rr_kp(
    ...
    We use logP as the compound-specific proxy for the PC term in the equation.
    """
    comp = TISSUE_COMPOSITION[tissue]
    pc = 10.0 ** float(logp)
    intr = compute_intracell_ionization(pka, p_hi_w)
    plas = compute_plasma_ionization(pka, p_hp)

    denom = 1.0 + 10.0 ** plas
    term_water = ((1.0 + 10.0 ** intr) / denom) * comp["Fiw"]
    term_lipid = (pc * comp["Fnl"] + (0.3 * pc + 0.7) * comp["Fnp"]) / denom
...
def nca_metrics(t: Sequence[float], c: Sequence[float]) -> Dict[str, float]:
...
def build_excel_export(df: pd.DataFrame, params: MouseParams, phys: Dict[str, float], kps: Dict[str, float], hep: Dict[str, float], pk: Dict[str, float]) -> bytes:
    buf = BytesIO()
...
with st.sidebar:
    ...
    bw_g = st.number_input("Mouse body weight (g)", min_value=10.0, max_value=60.0, value=25.0, step=0.5)
    pka = st.number_input("pKa", min_value=0.0, max_value=14.0, value=7.4, step=0.1)
    logp = st.number_input("logP", min_value=-2.0, max_value=8.0, value=2.5, step=0.1)
    fup = st.number_input("Fraction unbound in plasma (fu/Fup)", min_value=0.0001, max_value=1.0, value=0.1, step=0.01, format="%.4f")
    rbp = st.number_input("Blood-to-plasma ratio (Rbp)", min_value=0.1, max_value=5.0, value=1.5, step=0.1)

    st.subheader("Microsomal stability")
    tier = st.selectbox("Microsomal t1/2 tier", ["Low (1-30 min)", "Medium (30-60 min)", "High (60-120 min)", "Custom"])
    if tier == "Custom":
    ...
    st.subheader("Dosing")
    route = st.selectbox("Route", ["Oral", "IV"])
    oral_dose_mg = st.number_input("Oral dose (mg)", min_value=0.0, value=10.0, step=0.5)
    iv_dose_mg = st.number_input("IV dose (mg)", min_value=0.0, value=5.0, step=0.5)
    dose_interval_h = st.number_input("Dose interval (h)", min_value=0.5, max_value=240.0, value=24.0, step=0.5)
    n_doses = st.number_input("Number of doses", min_value=1, max_value=60, value=1, step=1)
    simulation_days = st.number_input("Simulation horizon (days)", min_value=1.0, max_value=365.0, value=7.0, step=1.0)
...
pk = nca_metrics(df["time_h"].to_numpy(), df[display_series].to_numpy())


# -----------------------------------------------------------------------------
# Main tabs
# -----------------------------------------------------------------------------
tab_sim, tab_pk, tab_kp, tab_about = st.tabs(["Simulation", "PK summary", "Kp / clearance", "About"])
...
with tab_sim:
    ...
    with left:
        ...
        if obs_df is not None and obs_time_col and obs_value_col:
...
with tab_pk:
    ...
    xlsx = build_excel_export(df, params, phys, kps, hep, pk)
    c1, c2 = st.columns(2)
    with c1:
        st.download_button(
...
with tab_kp:
    st.subheader("Rowland-Rodgers Kp estimates")
